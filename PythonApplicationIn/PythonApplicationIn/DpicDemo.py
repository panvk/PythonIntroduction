import os                   #operation
                            #system（操作系统）的缩写，这个库是对操作系统的封装，os.path对文件路径（文件夹）的操作：增加、删除、修改、删除
import requests             #requests（要求）爬虫库 ，相关pip
from lxml import etree      # lxml是python的一个解析库，支持HTML和XML的解析，支持XPath解析方式，而且解析效率非常高，安装方式pip3 install
                            # lxml
import urllib               # 是一个收集了多个涉及 URL 的模块的包（url library即 url图书馆）
 # 需要替换两样参数 保存的文件目录 、爬取的网站
...
class MZiTu: 
    #封装参数:
    #- 下载网址 例如： https://www.mzitu.com/
    #一级分类： 搜索下载请按2、性感类请按3、日本类请按4、台湾类请按5、清纯类请按6
    class_search = "search"
    class_sex = "xinggan"
    class_japan = "japan"
    class_taiwan = "taiwan"
    ...
    def mkdir(self, dir_name, img_url_list):
        ...
        if os.path.isdir("G:\Python爬取\图片/" + finall_dir) != True:  # 如果保存的路径不存在
            os.makedirs(r"G:\Python爬取\图片/" + finall_dir)  # 如果不存在。我们将创立这个路径
        path = "G:\Python爬取\图片/" + finall_dir + "/"
        return path
